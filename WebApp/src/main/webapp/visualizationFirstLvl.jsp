...
<head>
<meta charset="UTF-8">
<title>一级指标数据可视化</title>
</head>
<body>
	<jsp:include page="base.jsp" />

	<jsp:useBean id="resultByCountry"
		type="beans.firstLvlIndicatorCountryBean" scope="session"></jsp:useBean>
...
<script>
	...
	<%for (int i = 0; i < resultByCountry.getSize(); i++) {%>
		...
		<%for (int j = 0; j < resultByCountry.get(i).getSize(); j++) {%>
			temp.push(<%=resultByCountry.get(i).get(j).getScore()%>);
		<%}%>
			score["<%=resultByCountry.get(i).getSearchCondition()%>"] = temp;
